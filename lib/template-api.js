...
module.exports = function (Runtime, run, runtimeModelMap, relativeFileName) {
  ...
  const attachRuntimeEnvironment = (runtime, runtimeEnvironment) => {
    ...
    runtime.withResourceLoader((runtime, resourceName, resourceParameters) => {
      ...
      const componentNames = Object.keys(runtimeEnvironment.components || {});
      // Find the componentName with ends with the given resourceType
      const resource = componentNames.find(
        (componentName) =>
          resourceType.substr(-resourceType.length) === componentName
      );
      if (!resource) {
        return "";
      }
      return runtimeEnvironment.components[resource](runtimeEnvironment);
    });
  };
